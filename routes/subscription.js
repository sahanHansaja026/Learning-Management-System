...
const Enrollment = require("../models/createstudent");

// Route to enroll a user
router.post("/subscripion", async (req, res) => {
...
router.post("/check-subscripion", async (req, res) => {
  const { card_id, email } = req.body;
...
router.get("/subscripion/:email", async (req, res) => {
  ...
  try {
    ...
    if (enrollments.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No enrollments found for this email.",
      });
    }
...
router.get("/subscripion-count/:card_id", async (req, res) => {
...
router.get("/subscripion/recent", async (req, res) => {
  try {
