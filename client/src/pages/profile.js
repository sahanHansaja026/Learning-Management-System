...
class Profile extends Component {
  ...
  fetchExistingProfile = async (email) => {
    try {
      const res = await axios.get(`http://localhost:9001/profiles?email=${email}`);
      if (res.data.success) {
        ...
        this.setState({
          ...
          newPost: {
            ...this.state.newPost,
            ...userProfile,
            imageUrl: userProfile.image ? `http://localhost:9001/Profileimge/${userProfile.image}` : "",
          },
        });
      }
    } catch (error) {
  ...
  handleSubmit = (event) => {
    ...
    const request = this.state.existingPost
      ? axios.put("http://localhost:9001/profile/update/email", formData)
      : axios.post("http://localhost:9001/profile/save", formData);
